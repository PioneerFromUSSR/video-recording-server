        ...
        function (err) {
            if (err) {
                ...
                if (err.name === 'SCREEN_EXTENSION_NOT_INSTALLED') {
                    if (confirm("Chrome нужно специальное расширение для записи видео с экрана компьютера. Нажмите OK чтобы установить его")) {
...
var pauseResumeRecording = function () {
    if (!window['paused']) {
        ...
        $('#record-pause-resume').html('Продолжить');
        $('#post-video video').get(0).pause();
    } else {
        window['recorder'].resume();
        $('#record-pause-resume').html('Пауза');
